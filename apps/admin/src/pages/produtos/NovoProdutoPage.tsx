...
const EMPTY_BASICOS: BasicosForm = {
  name: '',
  sku: '',
  barcode: '',
  unit: '',
...
const EMPTY_DESCRICAO: DescricaoForm = {
  short_description: '',
  description: '',
}

const EMPTY_PRECO: PrecificacaoForm = {
  cost_price: '',
  sale_price: '',
  promotional_price: '',
...
export function NovoProdutoPage() {
  ...
  // Load categorias and marcas
  useEffect(() => {
    api
      .get<CategoryFlat[]>('/categories/flat')
      .then((r) => setCategorias(r.data.map((c) => ({ id: c.id, name: c.name }))))
      .catch(() => {})

    api
      .get<BrandPage>('/brands', { params: { limit: 100 } })
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span className="font-medium text-foreground">
              {basicos.name.trim() || 'Novo produto'}
            </span>
          </div>
          {productId && !published && (
        ...
        <Button
          ...
          disabled={!productId || publishing || published}
          className="gap-2"
        >
          {published ? (
            <>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-3xl px-6 py-8">
          {/* Empty state when no product yet */}
          {!productId && (
...
function FormCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-card shadow-sm">
      <div className="divide-y divide-border/60 px-6 py-5">
        {error && (
