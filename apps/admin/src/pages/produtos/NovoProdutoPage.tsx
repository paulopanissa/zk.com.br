...
export function NovoProdutoPage() {
  ...
  async function generateSeo() {
    ...
    try {
      await api.post(`/products/${productId}/seo/generate`)
      setSeoQueued(true)
    } finally {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between border-b border-border bg-background px-6 py-4">
        ...
        <Button onClick={handlePublish} disabled={!productId || publishing || published} className="gap-2">
          {published ? <><Check className="h-4 w-4" />Publicado</> : 'Publicar produto'}
        </Button>
      </div>
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabKey)}>
              ...
              <TabsList variant="line" className="mb-6 w-max min-w-full justify-start gap-1 border-b border-border pb-0">
                {TABS.map(({ key, label }) => (
                  <TabsTrigger
                    ...
                    className="relative whitespace-nowrap pb-3 px-3"
                  >
                    <span>{label}</span>
                    {savedTabs[key] && (
            ...
            <TabsContent value="basicos">
              <FormCard
                ...
                saveLabel={productId ? 'Salvar alterações' : 'Criar rascunho'}
              >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field label="CFOP" hint="4 ou 5 dígitos — Código Fiscal de Operações">
                      <Input
                        placeholder="5102"
                        value={fiscal.cfop}
                        onChange={(e) => setF('cfop', e.target.value.replace(/\D/g, '').slice(0, 5))}
                        inputMode="numeric"
                        maxLength={5}
                      />
                    </Field>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="CEST" hint="7 dígitos — opcional">
                      <Input
                        placeholder="0000000"
                        value={fiscal.cest}
                        onChange={(e) => setF('cest', e.target.value.replace(/\D/g, '').slice(0, 7))}
                        inputMode="numeric"
                        maxLength={7}
                      />
                    </Field>
              ...
              <FormCard error={tabErrors.seo} onSave={saveSeo} saving={saving.seo}>
                <FormSection title="Meta Tags" hint="Dados exibidos no Google e outros mecanismos de busca">
                  <Field label="Título SEO" trailing={<CharCount value={seo.seo_title} max={70} />}>
                    <Input
                      placeholder="Título para motores de busca (máx. 70 caracteres)"
                      value={seo.seo_title}
                      onChange={(e) => setS('seo_title', e.target.value)}
                      maxLength={70}
                    />
                  </Field>
                  ...
                  <Field label="Palavras-chave" hint="Separe por vírgula: ração gato, ração adulto, royal canin">
                    <Input
                      ...
                      onChange={(e) => setS('seo_keywords', e.target.value)}
                    />
                  </Field>
                </FormSection>

                <FormSection title="Schema.org JSON-LD" hint="Dados estruturados para rich results no Google. Use o schema de Product.">
